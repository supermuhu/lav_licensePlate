...
@section('content-header')
    @include('administrator.components.content-header', [
        'route' => route('license-plates'),
        'name' => 'Biển số',
        'key' => 'Danh sách',
    ])
@endsection
@section('title')
    <title>Admin License plate</title>
@endsection
@section('content')
            ...
            <div class="container">
                <div class="row mb-4">
                    <div class="h-100 col-md-6 d-flex align-items-center">
                        ...
                        <a href="{{ route('license-plates') }}" class="btn btn-danger"><i class="fa-solid fa-brush"></i></a>
                    </div>
                    <div class="col-md-6">
                        <div class="w-100 d-flex justify-content-end align-items-center">
                            ...
                            <a href="{{ route('license-plates.export') }}" class="btn btn-custom ms-1"><i
                        ...
                        <table class="table table-striped table-bordered table-custom align-middle">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col-width-100">Ký tự</th>
                                    <th class="col-width-100" scope="col">Hình ảnh</th>
                                    <th class="col-width-100" scope="col">Định giá</th>
                                    <th class="col-width-100" scope="col">Người dùng</th>
                                ...
                                @foreach ($licensePlates as $licensePlate)
                                    <tr>
                                        <td scope="col"><input type="checkbox" class="row-checkbox"
                                                data-id={{ $licensePlate->id }} value="{{ $licensePlate->id }}"></td>
                                        <td scope="col">{{ $licensePlate->id }}</td>
                                        <td scope="col" class="w-25">
                                            {{-- <input class="input-name bg-secondary-subtle p-2 w-100 d-none"
                                                type="text" value="{{ $licensePlate->license_number }}" name="license_number"> --}}
                                            <p class="p-0 m-0 data-name">{{ $licensePlate->license_number }}</p>
                                        </td>
                                        <td scope="col">
                                            <div class="col-lg-8">
                                        ...
                                        <td scope="col">
                                            {{-- <input class="input-name bg-secondary-subtle p-2 w-100 d-none"
                                                type="text" value="{{ $licensePlate->price }}" name="price"> --}}
                                            <p class="p-0 m-0 data-price">{{ $licensePlate->price }}</p>
                                        </td>
                                        <td scope="col" class="w-25">
                                            {{-- <input class="input-name bg-secondary-subtle p-2 w-100 d-none"
                                                type="text" value="{{ $licensePlate->user->name }}" name="user_id"> --}}
                                            <p class="p-0 m-0 data-user"> {!! nl2br(optional($licensePlate->user)->name . "\n" . optional($licensePlate->user)->phone) !!}</p>
                                        </td>
                                        <td scope="col">
                                            {{ $licensePlate->created_at }}
                                        </td>
                                        {{-- {!! nl2br(e(optional($item->user)->name . "\n" . optional($item->user)->phone . "\n" . optional($item->user)->email)) !!} --}}
                                        {{-- <td scope="col">
...
@section('script')
    <script>
        var queryUrl = "{{ route('license-plates.query', ['query' => ':query']) }}";
        var reloadUrl = "{{ route('license-plates') }}";
